...
class ImagePromptOptimizerService:
    ...
    async def _optimize_with_nodapi(
        ...
        *,
        ...
        system_prompt = (
            "你是一位专业的 AI 图片生成提示词优化师。你的任务是将用户的设计需求描述优化为高质量的图片生成提示词。\n"
            "要求：\n"
            "1. 必须保留用户原始名词和主体语义，不要把家具改成工业设备，不要把家居产品改成机械蓝图。\n"
            "2. 先理解用户输入中的核心词，再从参考提示词片段里只挑相关片段吸收，不要套固定模板。\n"
            "3. 如果参考片段与用户需求不一致，必须忽略不相关片段。\n"
            "4. 补充必要的画面质量描述词（光影、材质、构图、分辨率等），但不要改变品类。\n"
            "5. 输出为一段简洁有力的英文提示词，适合 Stable Diffusion / DALL-E / Midjourney 等模型。\n"
            "6. 最后用中文括号补充一段中文描述说明，方便国内用户理解。\n"
            "7. 直接输出优化后的提示词，不要输出多余解释、前缀或后缀。\n"
            "8. 提示词应紧扣输入词语，不允许出现与输入无关的大型机械、控制柜、工业装配等内容，除非用户明确要求。\n"
        )
